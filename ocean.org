...
ds_744['salt'].isel(time=0).plot()
#+END_SRC

#+RESULTS:
:RESULTS:
: <matplotlib.collections.QuadMesh at 0x7fac7cf4e9b0>
[[file:./figs_tmp/f1f782d7e105e0eb9bf7d88c861d4e4a1a2a608f.png]]
:END:
...
ds_744 = ds[['pot_temp','salt']].sel(z=slice(0,800))
print(ds_744)
#+END_SRC
...
Depth layers:  [   6.      21.      43.5     77.25   127.875  203.812  317.719  488.578
  744.867 1129.3   1705.95  2570.93  3868.39 ]
<xarray.Dataset>
Dimensions:      (time: 132, z: 9, y: 46, x: 72)
Coordinates:
  ,* time         (time) datetime64[ns] 2000-01-01 2000-02-01 ... 2010-12-01
  ,* x            (x) float32 -177.5 -172.5 -167.5 -162.5 ... 167.5 172.5 177.5
  ,* y            (y) float32 -90.0 -86.0 -82.0 -78.0 ... 78.0 82.0 86.0 90.0
  ,* z            (z) float32 6.0 21.0 43.5 77.25 127.9 203.8 317.7 488.6 744.9
    spatial_ref  int64 0
Data variables:
    pot_temp     (time, z, y, x) float32 dask.array<chunksize=(1, 9, 46, 72), meta=np.ndarray>
    salt         (time, z, y, x) float32 dask.array<chunksize=(1, 9, 46, 72), meta=np.ndarray>
Attributes:
    xlabel:   E1oM20_r01 (1880 atm.,the current modelE version)
    fromto:   From:  2000  JAN  1,  Hr  0      To:  2000  FEB  1, Hr  0  Mode...
#+end_example
